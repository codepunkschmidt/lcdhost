...
static int new_DirectFbSurface (lua_State *L)
{
    ...
    return new_Surface(L, LUACAIRO ".DirectFbSurface.mt", cs, CAIRO_SURFACE_TYPE_DIRECTFB, 1);
}
...
{
    luaL_newmetatable(L, LUACAIRO ".DirectFbSurface.mt");
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
    luaL_register(L, NULL, DirectFbSurface_lib_m);
    luaL_register(L, LUACAIRO ".DirectFbSurface", DirectFbSurface_lib_f);
    lua_newtable(L);
...
static int luaopen_lcairo_ud_directfb_surface (lua_State* L)
{
#if CAIRO_HAS_DIRECTFB_SURFACE
#if LUACAIRO_HAS_OO_INTERFACE
