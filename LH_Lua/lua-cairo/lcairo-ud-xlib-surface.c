...
static int new_XlibSurface (lua_State *L)
{
    ...
    return new_Surface(L, LUACAIRO ".XlibSurface.mt", cs, CAIRO_SURFACE_TYPE_XLIB, 1);
}
...
{
    luaL_newmetatable(L, LUACAIRO ".XlibSurface.mt");
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
    luaL_register(L, NULL, XlibSurface_lib_m);
    luaL_register(L, LUACAIRO ".XlibSurface", XlibSurface_lib_f);
    lua_newtable(L);
...
static int luaopen_lcairo_ud_xlib_surface (lua_State* L)
{
#if CAIRO_HAS_XLIB_SURFACE
#if LUACAIRO_HAS_OO_INTERFACE
