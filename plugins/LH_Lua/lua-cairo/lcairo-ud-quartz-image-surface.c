...
static int new_QuartzImageSurface (lua_State *L)
{
    ...
    return new_Surface(L, LUACAIRO ".QuartzImageSurface.mt", cs, CAIRO_SURFACE_TYPE_QUARTZ_IMAGE, 1);
}
...
{
    luaL_newmetatable(L, LUACAIRO ".QuartzImageSurface.mt");
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
    luaL_register(L, NULL, QuartzImageSurface_lib_m);
    luaL_register(L, LUACAIRO ".QuartzImageSurface", QuartzImageSurface_lib_f);
    lua_newtable(L);
...
static int luaopen_lcairo_ud_quartz_image_surface (lua_State* L)
{
#if CAIRO_HAS_QUARTZ_IMAGE_SURFACE
#if LUACAIRO_HAS_OO_INTERFACE
