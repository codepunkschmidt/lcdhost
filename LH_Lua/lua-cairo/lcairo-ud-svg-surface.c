...
static int new_SvgSurface (lua_State *L)
{
    ...
    return new_Surface(L, LUACAIRO ".SvgSurface.mt", cs, CAIRO_SURFACE_TYPE_SVG, 1);
}
...
{
    luaL_newmetatable(L, LUACAIRO ".SvgSurface.mt");
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
    luaL_register(L, NULL, SvgSurface_lib_m);
    luaL_register(L, LUACAIRO ".SvgSurface", SvgSurface_lib_f);
    lua_newtable(L);
...
static int luaopen_lcairo_ud_svg_surface (lua_State* L)
{
#if CAIRO_HAS_SVG_SURFACE
#if LUACAIRO_HAS_OO_INTERFACE
